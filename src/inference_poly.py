...
from utils import get_model, evaluate, predict, load_data, read_complete, load_ragtruth, predict_poly
...
def main(args):
    ...
    for filename, fulldata in data_list:
        ...
        poly_adapter_path = os.path.join(load_adapter_path,filename)
        model = PeftModel.from_pretrained(model, poly_adapter_path, adapter_name="poly", is_trainable=False)
        ...
        for test_id, data in tqdm(enumerate(fulldata), total=len(fulldata)):
            ...
            def get_pred(model, psgs):
                text = predict_poly(model, tokenizer, generation_config, 
                                        question, with_cot=args.with_cot, 
                                        passages=psgs, task_ids=test_id)
                pred = {
                    "test_id": test_id, 
                    "question": question, 
                    "answer": answer, 
