    ...
    **kwargs,
    ...
    for start_idx in tqdm.trange(
        ...
        results = self.es.lexical_multisearch(
            texts=queries[start_idx : start_idx + self.batch_size], top_hits=top_k
        )
...
def elasticsearch_lexical_multisearch(
...
def elasticsearch_hit_template(
...
if os.path.exists("/mnt/publiccache/huggingface"):
    tokenizer = AutoTokenizer.from_pretrained(
        "/mnt/publiccache/huggingface/Meta-Llama-3-8B-Instruct"
    )
else:
...
def bm25_retrieve(question, topk):
    docs_ids, docs = bm25_retriever.retrieve(
        [question], topk=topk, max_query_length=256
    )
